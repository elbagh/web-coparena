...
export async function guardarEquipo(
  ...
  if (idsABorrar.length > 0) {
    ...
    if (conEventos.length > 0) {
      const nombres = conEventos.map((j) => `${j.nombre} ${j.apellidos}`.trim()).join(", ");
      return noStore(
        {
          error: `No se puede quitar del equipo a quien ya tiene puntos anotados: ${nombres}. Habla con la organización.`
        },
        409
      );
    }
