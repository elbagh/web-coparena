 ...
 * nada.
 *
 * Sigue siendo UNA consulta: las subconsultas van dentro, y el 304 se sigue
 * respondiendo sin llegar a leer los partidos.
 */
export async function versionDirecto(db: D1Database): Promise<string> {
  const fila = await db
    ...
    .first<{ n: number; ultimo: string; marcador: number; ajustes: string; proximo: string }>();

  return [fila?.n ?? 0, fila?.marcador ?? 0, fila?.ultimo ?? "", fila?.ajustes ?? "", fila?.proximo ?? ""].join("-");
}
