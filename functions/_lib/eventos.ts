...
async function sentenciasSetNumero(
  ...
  let acumulado: EventoFila[] = [];
  for (const evento of eventos) {
    const antes = plegarEventos(acumulado, reglas).setNumero;
    ...
    acumulado = [...acumulado, evento];
    setTras.set(evento.orden, plegarEventos(acumulado, reglas).setNumero);
  }
...
export async function registrarCambio(
  ...
  if (entraId === saleId) throw new Error("Elige a dos personas distintas.");

  const alineacion = await leerAlineacion(db, partido.id);
  const sale = alineacion.find((fila) => fila.jugador_id === saleId);
  if (!sale) throw new Error("Quien sale ya no está en pista. Vuelve a cargar el partido.");
  if (alineacion.some((fila) => fila.jugador_id === entraId)) {
    throw new Error("Esa persona ya está en pista.");
  }
...
export async function deshacerCambio(db: D1Database, partido: PartidoAnotable): Promise<void> {
  const cambio = await db
    ...
    .bind(partido.id)
    .first<CambioFila>();
  if (!cambio) throw new Error("No hay ningún cambio que deshacer.");
  ...
  const sigueEnPista = await db
    ...
    .first<{ hay: number }>();
  if (!sigueEnPista) {
    throw new Error("Ese cambio ya no se puede deshacer: quien entró ha salido después.");
  }
