...
- Migrations live in [db/migrations/](db/migrations/) (`migrations_dir` in wrangler.toml). Main tables: `equipos`, `jugadores` (0002), `estadisticas` and `jugador_atributos` (0009); `equipos.capitan_jugador_id` (0011) replaced `owner_user_id`; the database allows a non-captain player's phone to be empty, though `validarRegistro` doesn't accept it yet (that's Task 2). The first version's tables (`inscripciones`, `reservas`, created in 0001) were dropped in 0008 — don't reintroduce them. Uniqueness is enforced via normalized columns (lowercase, accent-stripped in the endpoint — SQLite NOCASE is ASCII-only) plus UNIQUE indexes.
...
- **`estadisticas` is one row per load, not per player.** `partido_id IS NULL` is the manual load of the edición (all there is today); rows with a `partido_id` are what the coming match-result system will write. Everything read is `SUM(...)` over a player's rows, so that system lands without touching a single read query. The edición is **not** duplicated in the table: it comes from `jugadores.edicion_id` through the join, so moving someone between teams can't desync it. Metric names live once, in `_lib/estadisticas.ts` `METRICAS`.
...
- The trading card is [public/assets/cromo.js](public/assets/cromo.js) (`window.CopaCromo`), shared by `/mi-equipo/` and `/jugadores/`; `cromo.js` must be loaded **before** the page script. Both pages repeat the metric and attribute lists by hand (no bundler) — `test/unit/paridad-validacion.test.ts` fails when they drift from `_lib/estadisticas.ts` / `_lib/perfil.ts`.
