...
- Migrations live in [db/migrations/](db/migrations/) (`migrations_dir` in wrangler.toml). Main tables: `equipos`, `jugadores` (0002), `estadisticas` and `jugador_atributos` (0009); `equipos.capitan_jugador_id` (0011) replaced `owner_user_id`, and a non-captain player's phone is now optional, stored as an empty string. The first version's tables (`inscripciones`, `reservas`, created in 0001) were dropped in 0008 — don't reintroduce them. Uniqueness is enforced via normalized columns (lowercase, accent-stripped in the endpoint — SQLite NOCASE is ASCII-only) plus UNIQUE indexes.
